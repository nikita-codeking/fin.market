...
$arrErrors = Array();
$textSection = "";
$arrTemps = $_SESSION[$arParams["NAME"]][$arParams["IBLOCK_ID"]]["ITEMS"];
/*if(count(array_unique($idSections))>1){
    $index_session = count($_SESSION[$arParams["NAME"]][$arParams["IBLOCK_ID"]]["ITEMS"]);
    $arrTemps = Array();
    $this_pos = 1;
	foreach($_SESSION[$arParams["NAME"]][$arParams["IBLOCK_ID"]]["ITEMS"] as $keyS=>$itemS){
		if($this_pos!=$index_session){
            $arrTemps[$keyS] = $itemS;
		}
        $this_pos++;
	}
    ...
    $resSec = CIBlockSection::GetByID($idSections[0]);
    if($ar_res_sec = $resSec->GetNext())
        $textSection = $ar_res_sec['NAME'];
    $arrErrors[] = 'У вас уже добавлены к сравнению предложения по разделу '.$textSection.'.</br>Нет ничего невозможного, но сравнить можно только предложения из одной категории';
}*/
$_SESSION[$arParams["NAME"]][$arParams["IBLOCK_ID"]]["ITEMS"] = $arrTemps;
$arResult = $_SESSION[$arParams["NAME"]][$arParams["IBLOCK_ID"]]["ITEMS"];
$arResult['ERRORS'] = $arrErrors;
$arResult['ITEMS_SECTION'] = $resArrSections;
if(count($arResult['ITEMS_SECTION'] )>4){$arResult['ITEMS_SECTION'] = Array();}
