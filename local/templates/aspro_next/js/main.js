		...
		if(header.length)
		{
			...
			$(window).scroll(function(){
				...
				if(window.matchMedia('(min-width:768px)').matches)
				{
					...
					{
						/*if ((scrollTop + headerFixedHeight > tabs_offset.top) && headerCanFix) //kk 21.01.2020 added: "& headerCanFix"
						{
...
$(document).ready(function(){
	...
	if($mobileMenu.length){
		...
		$(document).on('click', '#mobileheader .burger', function(){
			SwipeMobileMenu();
			$("#mobileheader.mob_index").removeClass('fixed');
		})
...
if(!funcDefined('setHeightBlockSlider')){
	setHeightBlockSlider = function(){
		...
		if(iCountTabs <= 1)
		{
			...
			var tabsUnSelectShow = tabsContentUnhover-120;
			$(document).find('.specials.tab_slider_wrapp .tabs_content .tab.cur').attr('data-unhover', tabsContentUnhover);
			$(document).find('.specials.tab_slider_wrapp .tabs_content .tab.cur').attr('data-hover', tabsContentHover);
			//$(document).find('.specials.tab_slider_wrapp .tabs_content').height(tabsContentUnhover);
			$(document).find('.specials.tab_slider_wrapp .tabs_content').height(tabsUnSelectShow);
