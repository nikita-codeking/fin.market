...
if(!empty($PROP)):
    ...
    foreach($PROP as $prop_code=>$prop_fields):
        ...
        <tr id="tr_PROPERTY_<?echo $prop_fields["ID"];?>"<?if ($prop_fields["PROPERTY_TYPE"]=="F"):?> class="adm-detail-file-row"<?endif?>>
            ...
            /**
             * 030920
             * отобразим выбранные значения
             */
            $str_choose = '';
            if($prop_fields['PROPERTY_TYPE']=="G" && count($prop_fields['VALUE'])>0)
            {
                $str_choose = '</br></br><b>Выбранные значение:</b></br>';
                foreach ($prop_fields['VALUE'] as $itemChoose)
                {
                    $resSection = CIBlockSection::GetByID($itemChoose);
                    if($arSection = $resSection->GetNext())
                    {
                        $str_choose .= ' - ' . $arSection['NAME'] . '</br>';
                    }
            ...
            <td width="60%"><?_ShowPropertyField('PROP['.$prop_fields["ID"].']', $prop_fields, $prop_fields["VALUE"], (($historyId <= 0) && (!$bVarsFromForm) && ($ID<=0) && (!$bPropertyAjax)), $bVarsFromForm||$bPropertyAjax, 50000, $tabControl->GetFormName(), $bCopy);?><?=$str_choose?></td>
        </tr>
        <?
        $hidden = "";
