...
?>
<span style="top: 205px;
    z-index: 100;
    right: 100px;" id="form_element_35_settings_btn" class="adm-detail-settings adm-detail-settings-arrow adm-detail-settings-active" onclick="BX.adminShowMenu(this, [{'GLOBAL_ICON':'adm-menu-setting','TEXT':'Настройки','TITLE':'Настроить внешний вид списка','ONCLICK':'form_element_35.ShowSettings(\'/bitrix/admin/iblock_element_edit.php?mode=settings&amp;IBLOCK_ID=35&amp;type=aspro_next_catalog&amp;ID=<?echo $ID?>&amp;lang=ru&amp;find_section_section=<?=$IBLOCK_SECTION_ID?>&amp;WF=Y\')'},
{'ICONCLASS':'custom-fields-off','TEXT':'Отменить настройки формы','TITLE':'Отменить действие настроек полей формы на время сессии','ONCLICK':'form_element_35.DisableSettings();'}], {active_class: 'bx-settings-btn-active'});"></span>
<?
...
if(!empty($PROP)):
    ...
    /**
     * получим ID раздела
     * по нему узнаем какие свойства мы можем показывать
     */
    $secR = 0;
    $res = CIBlockElement::GetByID($ID);
    if($ar_res = $res->GetNext())
    {
        $secR = $ar_res['IBLOCK_SECTION_ID'];
    }
    $arPropertyForSection = array();
    $arPropertyForSectionRazdel = array();
    $arFilter = ['IBLOCK_ID' => 37, 'ACTIVE' => 'Y', 'PROPERTY_INCLUDE_GROUP' => $secR];
    $arSelect = [ 'ID', 'NAME', 'DATE_ACTIVE_FROM','PROPERTY_CODE_PROP','PROPERTY_RAZDEL','PROPERTY_ED_IZM'];
    $res = CIBlockElement::GetList(array(),$arFilter,false,false,$arSelect);
    while($itemRes = $res->Fetch()) {
        $arPropertyForSection[] = $itemRes['PROPERTY_CODE_PROP_VALUE'];
        $arPropertyForSectionRazdel[$itemRes['PROPERTY_CODE_PROP_VALUE']] = array('NAME_SECTION'=>$itemRes['PROPERTY_RAZDEL_VALUE'],'ED_IZM'=>$itemRes['PROPERTY_ED_IZM_VALUE']);
    }
    /**
     * мое меню
     */
    $thisURLAdmin = $APPLICATION->GetCurPageParam();

    /**
     * сгруппируем свойства
     */
    $tekst          = array();
    $usloviya       = array();
    $trebovaniya    = array();
    $harakteristiki = array();
    $servisi        = array();
    $documenti      = array();
    $other          = array();
    foreach($PROP as $prop_code=>$prop_fields):
        if(!in_array($prop_fields['CODE'],$arPropertyForSection)) continue;
        $nameRazel = $arPropertyForSectionRazdel[$prop_fields['CODE']]['NAME_SECTION'];
        //тексты
        if ($nameRazel == 'Текст')
        {
            $tekst[$prop_code] = $prop_fields;
            continue;
        }

        //условия
        if ($nameRazel == 'Условия')
        {
            $usloviya[$prop_code] = $prop_fields;
            continue;
        }

        //требования
        if ($nameRazel == 'Требования')
        {
            $trebovaniya[$prop_code] = $prop_fields;
            continue;
        }

        //характеристики
        if ($nameRazel == 'Характеристики')
        {
            $harakteristiki[$prop_code] = $prop_fields;
            continue;
        }

        //сервисы
        if ($nameRazel == 'Сервисы')
        {
            $servisi[$prop_code] = $prop_fields;
            continue;
        }

        //документы
        if ($nameRazel == 'Документы')
        {
            $documenti[$prop_code] = $prop_fields;
            continue;
        }

        //остальные
        $other[$prop_code] = $prop_fields;
    endforeach;



    echo '<div class="top_menu_admin_my">';

        if(count($tekst)>0):
            echo '<a href="'.$thisURLAdmin.'#tekst">Текст</a>';
        endif;

        if(count($usloviya)>0):
            echo '<a href="'.$thisURLAdmin.'#usloviya">Условия</a>';
        endif;

        if(count($trebovaniya)>0):
            echo '<a href="'.$thisURLAdmin.'#trebovaniya">Требования</a>';
        endif;

        if(count($harakteristiki)>0):
            echo '<a href="'.$thisURLAdmin.'#harakteristiki">Характеристики</a>';
        endif;

        if(count($servisi)>0):
            echo '<a href="'.$thisURLAdmin.'#servisi">Сервисы</a>';
        endif;

        if(count($documenti)>0):
            echo '<a href="'.$thisURLAdmin.'#documenti">Документы</a>';
        endif;
        
    echo '</div>';
    echo '
    <style>
    .top_menu_admin_my{padding-bottom: 6px;padding-top: 30px;text-align: right;}
    .top_menu_admin_my a{display: inline-block;margin-right: -1px;text-decoration: none;color: #3f4b54;font-weight: bold;padding: 8px 15px 0;height: 25px;border-radius: 4px 4px 0 0;border: 1px solid;border-color: #d2dbdd #bec6c8;}
    .top_menu_admin_my a:hover{background: #3f4b54;color: #fff;}
    </style>
    ';
    ...
    foreach ($usloviya as $key=>$item)
    {
        $PROP[$key] = $item;
    }
    foreach ($trebovaniya as $key=>$item)
    {
        $PROP[$key] = $item;
    }
    foreach ($harakteristiki as $key=>$item)
    {
        $PROP[$key] = $item;
    }
    foreach ($servisi as $key=>$item)
    {
        $PROP[$key] = $item;
    }
    foreach ($documenti as $key=>$item)
    {
        $PROP[$key] = $item;
    }
    $nameGroup = "";
    foreach($PROP as $prop_code=>$prop_fields):
         ...
         * группировка - начало
         */
        $newNameGroup = $arPropertyForSectionRazdel[$prop_fields['CODE']]['NAME_SECTION'];
        if (strlen($newNameGroup)>0)
        {
            if($nameGroup!=$newNameGroup)
            {
                $idGroup = "";
                if($newNameGroup=='Текст'){$idGroup = 'tekst';}
                if($newNameGroup=='Условия'){$idGroup = 'usloviya';}
                if($newNameGroup=='Требования'){$idGroup = 'trebovaniya';}
                if($newNameGroup=='Характеристики'){$idGroup = 'harakteristiki';}
                if($newNameGroup=='Сервисы'){$idGroup = 'servisi';}
                if($newNameGroup=='Документы'){$idGroup = 'documenti';}
                echo '<tr id="'.$idGroup.'" class="pop-group" title="'.$newNameGroup.'"><td colspan="2" style="text-align: center;font-weight: bold;">'.$newNameGroup.'</td></tr>';
        ...
        <tr id="tr_PROPERTY_<?echo $prop_fields["ID"];?>"<?if ($prop_fields["PROPERTY_TYPE"]=="F"):?> class="adm-detail-file-row"<?endif?>>
            ...
            <td width="60%"><?_ShowPropertyField('PROP['.$prop_fields["ID"].']', $prop_fields, $prop_fields["VALUE"], (($historyId <= 0) && (!$bVarsFromForm) && ($ID<=0) && (!$bPropertyAjax)), $bVarsFromForm||$bPropertyAjax, 50000, $tabControl->GetFormName(), $bCopy);?> <?=$arPropertyForSectionRazdel[$prop_fields['CODE']]['ED_IZM']?></td>
        </tr>
        <?
        $hidden = "";
...
?>
<style>
    #bx-admin-prefix .adm-detail-content-item-block table .heading{display: none;}
</style>
