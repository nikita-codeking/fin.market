			...
			if($arRegions === NULL)
			{
				...
				if($iRegionIBlockID = self::getRegionIBlockID())
				{
					...
					else
					{
						...
						$arItems = CNextCache::CIBLockElement_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => CNextCache::GetIBlockCacheTag($iRegionIBlockID), 'GROUP' => 'ID')), $arFilter, false, false, $arSelect);
		...
		public static function getCurrentRegion(){
			...
			if(!isset($arRegion)){
				...
				if($arRegions = self::getRegions()){
					...
					// get city
					$city = $bUseBitrixGeoIP = $bUseAltasibGeoIP = false;
					...
					if(!$city){
						if(\Bitrix\Main\Loader::includeModule('altasib.geoip')){
							...
							if($arData = ALX_GeoIP::GetAddr($ip)){
								...
								$bUseAltasibGeoIP = true;
							}
						}
					}
					$bUseGeoIP = $bUseBitrixGeoIP || $bUseAltasibGeoIP;
					...
					if($arTheme['REGIONALITY_TYPE'] === 'ONE_DOMAIN'){
						...
						foreach($arRegions as $arItem){
							if(in_array($_SERVER['SERVER_NAME'], $arItem['LIST_DOMAINS']) || in_array($_SERVER['HTTP_HOST'], $arItem['LIST_DOMAINS'])){
								return $arRegion = $arItem;
							}
						}
					}

					// region not finded, search by city name
					if(!$bFindDomain){
						if($city){
					...
					if(!$bFindDomain){
						return $arRegion = reset($arRegions);
					}
				}
				else{
					return false;
