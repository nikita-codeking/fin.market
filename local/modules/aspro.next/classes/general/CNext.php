	...
	public static function Start($siteID){
		...
		if(CModule::IncludeModuleEx(self::moduleID) == 1)
		{
			...
			if(!$bIndexBot)
			{
				...
				$APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH.'/js/swiper/swiper.min.js');
	...
	public static function SetMeta(){
		...
		foreach(self::$arMetaParams as $metaName => $metaValue)
		{
			if(strlen($metaValue = strip_tags($metaValue)))
			{
                //kk-->
                /**
                 * если нет картинки то ничего не выводим - для разделов
                 * для статических фиксированная картинка
                 */
                if($metaName === 'og:image'){
                    $pos_next = strpos($logo, '/CNext/');
                    //echo '<h1>'.$APPLICATION->GetCurPage(false).'</h1>';
                    if(strpos($APPLICATION->GetCurPage(false),'/catalog/')===false){
                        $is_catalog = false;
                    }else{
                        $is_catalog = true;
                    }
                    if(strpos($APPLICATION->GetCurPage(false),'/catalog/offers/')===false){
                        $is_catalog_offers = false;
                    }else{
                        $is_catalog_offers = true;
                    }
                    if($pos_next>0){
                        if(!$is_catalog){
                            $metaValue = SITE_DIR.'/upload/og.jpg';
                        }else{
                            if(!$is_catalog_offers){
                                continue;
                            }else{
                                $metaValue = SITE_DIR.'/upload/og.jpg';
                            }
