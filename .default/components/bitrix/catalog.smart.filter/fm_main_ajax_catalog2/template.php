...
$pos_strahovki = strpos($this_url, '/catalog/strakhovanie/');
if($arResult["ITEMS"] && $pos_strahovki===false){?>
        ...
        <div class="bx_filter_section">
            <form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
                ...
                <div class="bx_filter_parameters">
                    ...
                    $pos_point_in_name = $arItem['MAIN_GROUP'];
                    //echo 'point position - ' . $pos_point_in_name;
                    if(strlen($pos_point_in_name)>0){
                        $group_filters  = $pos_point_in_name;
                    }else{

                        //ВЫВОДИМ - ПОДГРУППИРОВКУ - НАЧАЛО
                        if($arItem['GROUPS'])
                        {
                            foreach ($arItem['GROUPS'] as $keyG => $itemGroup):?>
                                <?php
                                /**
                                 * проверка - надо ли выводить подгруппу
                                 */
                                $elCountGr = 0;
                                foreach ($itemGroup as $elementGroup):
                                    if(empty($elementGroup["VALUES"]) || isset($elementGroup["PRICE"]))
                                        continue;

                                    if ($elementGroup["DISPLAY_TYPE"] == "A" && ($elementGroup["VALUES"]["MAX"]["VALUE"] - $elementGroup["VALUES"]["MIN"]["VALUE"] <= 0))
                                        continue;
                                    $elCountGr++;
                                endforeach;
                                ?>
                                <?if($elCountGr > 0):?>
                                    <div class="bx_filter_parameters_box inner podgr" data-expanded="N" data-prop_code="" data-property_id="">
                                        <span data-f="Показать" data-fi="товар" data-fr="товара" data-frm="товаров" class="bx_filter_container_modef"></span>
                                        <div class="bx_filter_parameters_box_title inner icons_fa">
                                            <div>
                                                <b><?= $keyG ?></b>
                                                <div class="char_name">
                                        ...
                                        <div class="bx_filter_block limited_block new_groups podgruppa" style="display: none;">
                                            <?
                                            foreach ($itemGroup as $elementGroup):
                                                /*if(
                                                    empty($elementGroup["VALUES"])
                                                    || isset($elementGroup["PRICE"])
                                                )
                                                    continue;
                                                */
                                                if (
                                                    $elementGroup["DISPLAY_TYPE"] == "A"
                                                    && (
                                                        $elementGroup["VALUES"]["MAX"]["VALUE"] - $elementGroup["VALUES"]["MIN"]["VALUE"] <= 0
                                                    )
                                                )
                                                    continue;
                                                if(empty($elementGroup["VALUES"]))
                                                {
                                                    $invise = true;
                                                }
                                                else
                                                ...
                                                printEl($elementGroup,$arParams,$numVisiblePropValues,$arResult,$APPLICATION,$open_group_filters,$key,$invise);
                                            endforeach;
                                            ?>
                                        </div><!--.bx_filter_block.limited_block.new_groups-->
                    ...
                    <?
                    $this_url = $APPLICATION->GetCurPage();
                    $docPage = strpos($this_url, 'debetovye_karty');
                    ...
                    <div class="bx_filter_parameters_box <?if($docPage > 0 && $group_filters == 'Документы' || $rkoPage > 0 && $group_filters == 'Документы' || $harPage > 0 && $group_filters == 'Характеристики' || $avtoCreditPage > 0 && $group_filters == 'Характеристики' || $avtoCreditPage > 0 && $group_filters == 'Сервисы'){echo 'ifclasses';}else{'';}?>" data-expanded="N" data-prop_code="" data-property_id="">
                        ...
                        <div class="bx_filter_block limited_block new_groups" style="display: none;">
                            <?$open_group_filters = true;
                            }//if(strlen($group_filters)>0 && $group_filters!=$old_group_filters)

                            if(!$open_group_filters):?>
                            <div class="bx_filter_parameters_box <?=$class;?> invise" data-expanded="<?=($arItem["DISPLAY_EXPANDED"] ? $arItem["DISPLAY_EXPANDED"] : "N");?>" data-prop_code="<?=strtolower($arItem["CODE"]);?>" data-property_id="<?=$arItem["ID"]?>">
                                <span data-f="<?=Loc::getMessage('CT_BCSF_SET_FILTER')?>" data-fi="<?=Loc::getMessage('CT_BCSF_SET_FILTER_TI')?>" data-fr="<?=Loc::getMessage('CT_BCSF_SET_FILTER_TR')?>" data-frm="<?=Loc::getMessage('CT_BCSF_SET_FILTER_TRM')?>" class="bx_filter_container_modef"></span>
                                ...
                                printEl($arItem,$arParams,$numVisiblePropValues,$arResult,$APPLICATION,$open_group_filters,$key,$invise);
                                ?>
                                ...
                                <div class="bx_filter_button_box active">
                                    <div class="bx_filter_block">
                                        <div class="bx_filter_parameters_box_container">
                                            ...
                                            <button class="bx_filter_search_button btn btn-default" type="submit" id="set_filter" name="set_filter" data-href="" value="" >
...
<?
function printEl($arItem,$arParams,$numVisiblePropValues,$arResult,$APPLICATION,$open_group_filters,$key,$invise)
{
    $style="";
...
<div class="bx_filter_block <?=($arItem["PROPERTY_TYPE"]!="N" && ($arItem["DISPLAY_TYPE"] != "P" && $arItem["DISPLAY_TYPE"] != "R") ? "limited_block" : "");?> <?if($invise==true):?>invise<?endif;?>" <?=$style;?>>
    <div class="bx_filter_parameters_box_container <?=($arItem["DISPLAY_TYPE"]=="G" ? "pict_block" : "");?>">
        <?if($open_group_filters):?>
            <?php
            $name_prop = $arItem["NAME"];
            $pos_sl = strpos($name_prop, "/");
            if($pos_sl>0)
            {
                $name_prop = substr($name_prop,0,$pos_sl);
            }
            ?>
            <?
            $edIzm = "";
            ...
            {
                $edIzm = $itemResP['PROPERTY_ED_IZM_VALUE'];
            }
            ?>
            <?$vis_name=true;?>
            <?php
            ...
            <?if($vis_name==true):?>
                <?if(strlen($edIzm)>0):?>
                    <div class="prop_horizont"><?=$name_prop?>, <?=$edIzm?></div>
                <?else:?>
                    <div class="prop_horizont"><?=$name_prop?></div>
                <?endif;?>
            <?endif;?>
        <?endif;?>
        <?
        ...
        switch ($arItem["DISPLAY_TYPE"]){
            case "A"://NUMBERS_WITH_SLIDER
                ...
                <div class="wrapp_all_inputs wrap_md">
                    ...
                    <div class="wrapp_change_inputs iblock" style="display: none;">
                        <div class="bx_filter_parameters_box_container_block">
                            <div class="bx_filter_input_container form-control bg">
                                <input
                                        class="min-price"
                                        ...
                                        value="<?echo $value1?>"
                                        size="5"
                                        placeholder="<?echo formatToHuman($value1);?>"
                                        onkeyup="smartFilter.keyup(this)"
                                />
                            </div>
                        </div>
                        <div class="bx_filter_parameters_box_container_block">
                            <div class="bx_filter_input_container form-control bg">
                                <input
                                        ...
                                        value="<?echo $value5?>"
                                        size="5"
                                        placeholder="<?echo formatToHuman($value5);?> - 1"
                                        onkeyup="smartFilter.keyup(this)"
                                />
                            </div>
                        </div>
                    ...
                    <div class="wrapp_slider iblock">
                        <div class="bx_ui_slider_track" id="drag_track_<?=$key?>">
                            ...
                            <div class="bx_ui_slider_part first p1"><span id="first_value_filter"><?=formatToHuman($value1)?></span></div>
                            <div class="hiddenBlockMin" style="display: none;"><?echo $value1;?></div>
                            ...
                            <div class="bx_ui_slider_part last p5"><span id="last_value_filter"><?=formatToHuman($value5)?></span></div>
                            <div class="hiddenBlockMax" style="display: none;"><?echo $value5;?></div>
            ...
            default://CHECKBOXES
                ...
                <label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label <?=($isSize ? "nab sku" : "");?> <?=($i==$count ? "last" : "");?> <? echo $ar["DISABLED"] ? 'disabled': '' ?>" for="<? echo $ar["CONTROL_ID"] ?>">
												<span class="bx_filter_input_checkbox">
                                                    ...
                                                    if(count($arItem["VALUES"])==1)
                                                    {
                                                        $valArValue = $name_prop;
                                                    }
                                                    ?>
													<span class="bx_filter_param_text" title="<?=$ar["VALUE"];?>"><?=$valArValue;?><?
...
<script>
    $(document).ready(function(){
        $('.bx_ui_slider_handle.left').click(function() {
            var firstValue = $(this).parent().parent().children(".first").children('#first_value_filter');
            var inputValueFirst = $(this).parent().parent().parent().parent().children('.wrapp_change_inputs').children('.bx_filter_parameters_box_container_block:nth-child(1)').children().children('.min-price').val();
            console.log(inputValueFirst);
            if(inputValueFirst >= 10000 && inputValueFirst < 1000000){
                var firstRoundValueInput =  Math.round((inputValueFirst / 1000)) + ' ' + 'тыс.';
                firstValue.text(firstRoundValueInput);
            }
            if(inputValueFirst >= 1000000){
                var firsRoundValueInput =  Math.round((inputValueFirst / 1000000)) + ' ' + 'млн.';
                firstValue.text(firstRoundValueInput);
            }
        });
       /* setInterval(function(){
            $('.bx_ui_slider_handle.left').each(function(){
            var setFirstValue =  $(this).parent().parent().children(".first").children('#first_value_filter');
            var setInputValueFirst =  $(this).parent().parent().parent().parent().children('.wrapp_change_inputs').children('.bx_filter_parameters_box_container_block:nth-child(1)').children().children('.min-price').val();
                console.log(setInputValueFirst);
            if(setInputValueFirst >= 10000 && setInputValueFirst < 1000000){
                    var firstRoundValueInput =  Math.round((setInputValueFirst / 1000)) + ' ' + 'тыс.';
                    setFirstValue.text(firstRoundValueInput);
                }
                if(setInputValueFirst >= 1000000){
                    var firsRoundValueInput =  Math.round((setInputValueFirst / 1000000)) + ' ' + 'млн.';
                    setFirstValue.text(firstRoundValueInput);
                }
            })
        ...
        $('.bx_ui_slider_handle.right').click(function() {
            var lastValue = $(this).parent().parent().children(".last").children('#last_value_filter');
            var inputValueLast = $(this).parent().parent().parent().parent().children('.wrapp_change_inputs').children('.bx_filter_parameters_box_container_block:nth-child(2)').children().children('.max-price').val();
            console.log(inputValueLast);
            if(inputValueLast >= 10000 && inputValueLast < 1000000){
                var lastRoundValueInput = Math.round((inputValueLast / 1000)) + ' ' + 'тыс.';
                lastValue.text(lastRoundValueInput);
            }
            if(inputValueLast >= 1000000){
                var lastRoundValueInput = Math.round((inputValueLast / 1000000)) + ' ' +'млн.';
                lastValue.text(lastRoundValueInput);
            }

        });
        /*setInterval(function(){
            $('.bx_ui_slider_handle.right').each(function () {
                if($(this).attr('style')=='right: 0%;')
                {
                    console.log($(this).parent().parent().children('.hiddenBlockMax').text());
                }
            });
        },1000);*/
        /*$('.bx_ui_slider_track').focusout(function () {
            console.log($(this).children('.hiddenBlockMin').text());
            console.log($(this).children('.hiddenBlockMax').text());
        });*/
        /*setInterval(function(){
            $('.bx_ui_slider_handle.right').each(function(){
                var setLastValue =  $(this).parent().parent().children(".first").children('#last_value_filter');
                var setInputValueLast =  $(this).parent().parent().parent().parent().children('.wrapp_change_inputs').children('.bx_filter_parameters_box_container_block:nth-child(2)').children().children('.max-price').val();
                console.log(setInputValueLast);
                },1000);}*/
    });
</script>
