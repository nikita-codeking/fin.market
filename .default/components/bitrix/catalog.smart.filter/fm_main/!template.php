...
if($arResult["ITEMS"]){?>
    <div class="bx_filter bx_filter_vertical swipeignore 10">
        <div class="bx_filter_section">
            <form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
                ...
                <div class="bx_filter_parameters">
                            ...
                            <div class="bx_filter_parameters_box_container ">
                                ...
                                if($pos_credit_cart>0 || $pos_credit_nal>0 || $pos_zaymi>0 || $pos_autocredity>0 || $pos_ipoteka>0 || $pos_refinansirovanie>0 || $pos_debit_cart>0):
                                    ...
                                    <div class="sort_filter mobile_filter_compact">
                                        <?if($pos_debit_cart>0):?>
                                            <a href="<?=$this_url?>?sort=property_OT_DO_GOD_OBSL_MAX&method=<?if($_GET["method"]=='desc'):?>asc<?else:?>desc<?endif;?>" class="sort_btn <?if($_GET["sort"] == "property_OT_DO_GOD_OBSL_MAX"):?>current <?=$_GET["method"]?><?else:?>desc<?endif;?>" rel="nofollow">
                                                <i class="icon" title="годовое обслуживание"></i><span>Годовое обслуживание</span><i class="arr icons_fa"></i>
                                            </a>
                                        <?else:?>
                                            <a href="<?=$this_url?>?sort=property_OT_DO_CREDIT_LIM_MAX&method=<?if($_GET['method']=='desc'):?>asc<?else:?>desc<?endif;?>" class="sort_btn <?if($_GET["sort"] == "property_OT_DO_CREDIT_LIM_MAX"):?>current <?=$_GET["method"]?><?else:?>desc<?endif;?>" rel="nofollow">
                    ...
                    $itemDocumentsCreated = false;
                    $itemStatementsCreated = false;
                    $posCounter = 0;
                    ...
                    foreach($arResult["ITEMS"] as $key=>$arItem)
                    {
                        ...
                        if($arItem["DISPLAY_TYPE"] == "F" and count($arItem["VALUES"]) == 1 and $arItem["CODE"] != "LINK_REGION"){
                            $checkboxInLine = true;
                        }

                        if ($arItem["CODE"] == "GRAZHDANSTVO_RF" or $arItem["CODE"] == "ZAYAVLENIE_ANKETA" or $arItem["CODE"] == "PASPORT" or $arItem["CODE"] == "SPRAVKA_PO_FORME_BANKA" or $arItem["CODE"] == "SPRAVKA_PO_FORME_2_NDFL"){
                            $checkboxInLine = false;
                            if($itemDocumentsCreated){
                                goto checkboxes_in_one_item;
                            }
                            $itemDocumentsCreated = true;
                        }elseif($itemDocumentsCreated){
                            goto group_in_one_item_end;
                            return_to_beginning_iteration1:
                            $itemDocumentsCreated = false;
                        }

                        $pos = strpos($arItem["NAME"], Loc::getMessage("STATEMENTS_SEARCH"));
                        if ($pos === false){
                            if($itemStatementsCreated){
                                goto group_in_one_item_end;
                                return_to_beginning_iteration2:
                                $itemStatementsCreated = false;
                            }
                        }else{
                            $checkboxInLine = false;
                        ...
                        <?if($checkboxInLine):?>
                            <div class="bx_filter_parameters_box_title icons_fa" id="checkbox_inline_id">
                                <?else:?>
                                <div class="bx_filter_parameters_box_title icons_fa" >
                                    <?endif;?>
                                    <div <?if($checkboxInLine):?>class="checkbox_inline_name"<?endif;?>>
                                        <?=str_replace("Магазин","Партнер",( $arItem["CODE"] == "MINIMUM_PRICE" ? Loc::getMessage("PRICE") : ($itemDocumentsCreated ? Loc::getMessage("DOCUMENTS") : ($itemStatementsCreated ? Loc::getMessage("STATEMENTS") : $arItem["NAME"]))));?>
                            ...
                            <?endif;?>
                                ...
                                <div class="bx_filter_block <?=($arItem["PROPERTY_TYPE"]!="N" && ($arItem["DISPLAY_TYPE"] != "P" && $arItem["DISPLAY_TYPE"] != "R") ? "limited_block" : "");?> <?if($checkboxInLine):?> checkbox_inline<?endif;?>" <?=$style;?>>
                                    <?checkboxes_in_one_item:?>
                                    <div class="bx_filter_parameters_box_container <?=($arItem["DISPLAY_TYPE"]=="G" ? "pict_block" : "");?>">
                                        <?
                                        ...
                                        switch ($arItem["DISPLAY_TYPE"]){
                                            ...
                                            default://CHECKBOXES
												...
												<span class="bx_filter_input_checkbox">
													...
													<span class="bx_filter_param_text" title="<?=$ar["VALUE"];?>"><?if($itemDocumentsCreated)echo $arItem["NAME"];elseif(!$checkboxInLine)echo $ar["VALUE"];?><?//=$ar["VALUE"];?><?
                                            ...
                                            <?} else{?>
                                                <?foreach($arItem["VALUES"] as $val => $ar):?>
                                                    <?if($ar["VALUE"]!=$GLOBALS["arRegion"]["NAME"])continue;?>
                                                    <?if($j > $numVisiblePropValues && !$isHidden):
                                                        $isHidden = true;?>
                                                        <div class="hidden_values">
                                                    <?endif;?>
                                                ...
                                                <?if($isHidden):?>
                                                    ...
                                                    <div class="inner_expand_text"><span class="expand_block"><?=Loc::getMessage("FILTER_EXPAND_VALUES");?></span></div>
                                                <?endif;?>
                                                <div class="accordion" id="allregion">Все регионы</div>
                                    ...
                                    <?if(($itemDocumentsCreated or $itemStatementsCreated) and count($arResult["ITEMS"])-1 <> $posCounter){
                                        continue;
                                    }?>
                                    <?group_in_one_item_end:?>
                                    <div class="clb"></div>
                                </div>
                                <?if($checkboxInLine):?>
                            </div>
                        ...
                        <?if($itemDocumentsCreated and count($arResult["ITEMS"])-1 <> $posCounter){
                            goto return_to_beginning_iteration1;
                        }?>
                        <?if($itemStatementsCreated and count($arResult["ITEMS"])-1 <> $posCounter){
                            goto return_to_beginning_iteration2;
                        }?>
                    <?}?>

                    <!--Вывод фильтров блока Документы-->
