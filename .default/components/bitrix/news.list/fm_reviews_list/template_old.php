				...
				<div class="items row">
					<?foreach($arItems['ITEMS'] as $key => $arItem):?>
						...
						<?if(isset($arItem['START_DIV']) && $arItem['START_DIV'] == 'Y'):?>
							<div class="z col-md-4 col-sm-4">
						<?endif;?>
							<div class="zz <?=((isset($arItem['CLASS']) && $arItem['CLASS']) ? $arItem['CLASS'] : 'col-md-4 col-sm-4');?>">
								<div class="item item_bottom shadow animation-boxs <?=($isWideBlock ? 'wide-block' : '')?> <?=($hasWideBlock ? '' : 'normal-block')?>"  id="<?=$this->GetEditAreaId($arItem['ID']);?>">
									<div class="inner-item">
										<?if($bImage):?>
											<div class="image shine">
												<?if($bDetailLink):?><a class="card" href="<?=$arItem['DETAIL_PAGE_URL']?>"><?endif;?>
										...
										<?endif;?>
										<div class="title item__gradient">
