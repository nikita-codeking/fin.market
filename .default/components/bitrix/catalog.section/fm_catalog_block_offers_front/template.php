                    ...
                    <div class="catalog_item item_wrap main_item_wrapper" id="<?=$this->GetEditAreaId($arItem['ID']);?>_<?=$arParams["FILTER_HIT_PROP"]?>">
                        <div class="inner_wrap">
                            ...
                            <div class="item_info <?=$arParams["TYPE_SKU"]?>">
                                ...
                                <?php
                                if($arItem["OFFERS"][0]["PROPERTIES"]["KREDITNYY_LIMIT"]["VALUE"] || $arItem["OFFERS"][0]["PROPERTIES"]["KREDITNYY_LIMIT_DO"]["VALUE"]){
                                    if($arItem["OFFERS"][0]["PROPERTIES"]["KREDITNYY_LIMIT"]["VALUE"] && $arItem["OFFERS"][0]["PROPERTIES"]["KREDITNYY_LIMIT_DO"]["VALUE"]){
                                        $kr_limit = min($arResult["OFFERS"][0]["PROPERTIES"]["KREDITNYY_LIMIT"]["VALUE"],$arItem["OFFERS"][0]["PROPERTIES"]["KREDITNYY_LIMIT_DO"]["VALUE"]);
                                    }elseif ($arItem["OFFERS"][0]["PROPERTIES"]["KREDITNYY_LIMIT"]["VALUE"]){
                                        $kr_limit = $arItem["OFFERS"][0]["PROPERTIES"]["KREDITNYY_LIMIT"]["VALUE"];
                                    }else{
                                        $kr_limit = $arItem["OFFERS"][0]["PROPERTIES"]["KREDITNYY_LIMIT_DO"]["VALUE"];
                                    }
                                }else{
                                    $kr_limit = 0;
                                }
                                $ar_limit = str_split($kr_limit,3);

                                if(count($ar_limit)>2){
                                    $kr_limit = (ceil($kr_limit/1000000))*1000000;
                                    $kr_limit = strval($kr_limit);
                                    $kr_limit = substr($kr_limit, 0,strlen($kr_limit)-6) . ' млн';
                                }elseif(count($ar_limit)>1){
                                    $kr_limit = (ceil($kr_limit/1000))*1000;
                                    $kr_limit = strval($kr_limit);
                                    $kr_limit = substr($kr_limit, 0,strlen($kr_limit)-3) . ' тыс';
                                }


                                if($arItem["OFFERS"][0]["PROPERTIES"]["PROTSENTNAYA_STAVKA"]["VALUE"] || $arItem["OFFERS"][0]["PROPERTIES"]["PROTSENTNAYA_STAVKA_DO"]["VALUE"]){
                                    if($arItem["OFFERS"][0]["PROPERTIES"]["PROTSENTNAYA_STAVKA"]["VALUE"] && $arItem["OFFERS"][0]["PROPERTIES"]["PROTSENTNAYA_STAVKA_DO"]["VALUE"]){
                                        $pr_stavka = min($arResult["OFFERS"][0]["PROPERTIES"]["PROTSENTNAYA_STAVKA"]["VALUE"],$arItem["OFFERS"][0]["PROPERTIES"]["PROTSENTNAYA_STAVKA_DO"]["VALUE"]);
                                    }elseif ($arItem["OFFERS"][0]["PROPERTIES"]["PROTSENTNAYA_STAVKA"]["VALUE"]){
                                        $pr_stavka = $arItem["OFFERS"][0]["PROPERTIES"]["PROTSENTNAYA_STAVKA"]["VALUE"];
                                    }else{
                                        $pr_stavka = $arItem["OFFERS"][0]["PROPERTIES"]["PROTSENTNAYA_STAVKA_DO"]["VALUE"];
                                    }
                                }else{
                                    $pr_stavka = 0;
                                }

                                if($arItem["OFFERS"][0]["PROPERTIES"]["LGOTNYY_PERIOD"]["VALUE"]){
                                    $lg_period = $arItem["OFFERS"][0]["PROPERTIES"]["LGOTNYY_PERIOD"]["VALUE"];
                                }else{
                                    $lg_period = 0;
                                }

                                $propety_store =
                                    '<div class="main_properties"><table class="props_list" style="margin: 0 auto;"><tbody>';
                                if($pr_stavka>0):
                                    $propety_store = $propety_store . '<tr itemprop="additionalProperty">
																 <td class="char_name">
																		<div class="props_item ">
																			 <span itemprop="name">Процентная ставка</span>
																		</div>
																 </td>
																 <td class="char_value"><span itemprop="value">' . $pr_stavka . ' %</span></td>
															</tr>';
                                endif;
                                if($kr_limit>0):
                                    $propety_store = $propety_store . '<tr itemprop="additionalProperty">
																<td class="char_name">
																	 <div class="props_item ">
																		  <span itemprop="name">Кредитный лимит</span>
																	 </div>
																</td>
																<td class="char_value"><span itemprop="value">' . $kr_limit . ' руб.</span></td>
															</tr>';
                                endif;
                                if($lg_period>0):
                                    $propety_store = $propety_store . '<tr itemprop="additionalProperty">
																 <td class="char_name">
																	   <div class="props_item ">
																			<span itemprop="name">Льготный период</span>
																	   </div>
																 </td>
																 <td class="char_value"><span itemprop="value">'.$lg_period.' дней</span></td>
															</tr>';
                                endif;
                                $propety_store = $propety_store . '</tbody></table></div>';
                                echo $propety_store;
                                ?>
