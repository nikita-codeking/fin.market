...
while($section_banners = $prop_banners->Fetch())
{
    if($section_banners['PROPERTY_MOBILE_VERSION_VALUE'] == 'Нет'){
        $arResult['BANNERS_COMPRESION']['DETAIL_TEXT_DESC'] = $section_banners['DETAIL_TEXT'];
    }
    elseif($section_banners['PROPERTY_MOBILE_VERSION_VALUE'] == 'Да'){
...
$arSelect = Array('ID','PROPERTY_MOBILE_VERSION', 'DETAIL_TEXT');
$arFilter = Array("IBLOCK_ID"=>46, "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y","PROPERTY_BANNERS_SECTION"=>$arResult['ORIGINAL_PARAMETERS']['SECTION_ID']);
$prop_banners_one = CIBlockElement::GetList(Array(), $arFilter, false, Array(), $arSelect);

while($section_banners_one = $prop_banners_one->Fetch())
{
    if($section_banners_one['PROPERTY_MOBILE_VERSION_VALUE'] == 'Нет'){
        $arResult['BANNERS_ONE_REQUEST']['DETAIL_TEXT_DESC'] = $section_banners_one['DETAIL_TEXT'];
    }
    elseif($section_banners_one['PROPERTY_MOBILE_VERSION_VALUE'] == 'Да'){
        $arResult['BANNERS_ONE_REQUEST']['DETAIL_TEXT_MOBILE'] = $section_banners_one['DETAIL_TEXT'];
    }
