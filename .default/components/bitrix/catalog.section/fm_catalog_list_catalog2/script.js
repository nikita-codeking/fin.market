$(document).ready(function(){
	...
	function cardTypePosition() {
		var ww = $(window).width();
		if(ww <= 550) {
			$('.image_block .card-data').each(function( i ) {
				var targetPosition = $(this).parents('.image_block').siblings('.description_wrapp');
				var cardData = $(this).detach();
				cardData.appendTo(targetPosition);
			});
		} else {
			$('.description_wrapp .card-data').each(function( i ) {
				var targetPosition = $(this).parents('.description_wrapp').siblings('.image_block').children('a');
				var cardData = $(this).detach();
				cardData.appendTo(targetPosition);
			});
		}
