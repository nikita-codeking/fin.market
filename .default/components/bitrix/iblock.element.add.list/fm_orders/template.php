...
<?if($arResult["NO_USER"] == "N"):?>
	<thead>
		<tr>
			<td colspan="4"><b><?=GetMessage("IBLOCK_ADD_LIST_TITLE")?></b></td>
		</tr>
	</thead>
	<tbody>
	<?if (count($arResult["ELEMENTS"]) > 0):?>
		...
		<tr>
            <td><?=$arElement["CREATED_DATE"]?></td>
			<td><!--a href="detail.php?CODE=<?=$arElement["ID"]?>"--><?=$arElement["NAME"]?><!--/a--></td>
			<td><small>
			<?
				$props = CIBlockElement::GetProperty(28, $arElement["ID"], array(), Array("CODE"=>"STATUS"));
				if($ar_props = $props->Fetch()) 
				{
					if ($ar_props["VALUE"] == 2127) echo 'Одобрена';
					else echo 'Отклонена';
				}
			?>
			</small></td>
