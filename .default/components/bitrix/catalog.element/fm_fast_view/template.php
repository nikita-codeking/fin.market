                ...
                <div class="sku_props">
                    <?if (!empty($arResult['OFFERS_PROP'])){?>
                        <div class="bx_catalog_item_scu wrapper_sku" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PROP_DIV']; ?>">
                            ...
                            <?
                            $center_button = 'left: 64px;position: absolute;';
                            $visible_srv = false;
                            ...
                            <?foreach ($arSkuTemplate as $code => $strTemplate){
                                ...
                                foreach($arResult["OFFERS"] as $itemOffer):
                                    if(strpos(trim($arResult["OFFERS"][0]["PROPERTIES"]["URL2"]["VALUE"]) ,'admitad') >-1){
                                        $referer=trim($arResult["OFFERS"][0]["PROPERTIES"]["URL2"]["VALUE"]) . "subid/" .$_COOKIE['utm_source'] ;

                                    }else{
                                        $referer=trim($arResult["OFFERS"][0]["PROPERTIES"]["URL2"]["VALUE"]) ."&sa=".$_COOKIE['utm_source'] ;
                                    }
                                    /*$referer = "";
                                    ...
                                    if(strlen(trim($itemOffer["PROPERTIES"]["URL2"]["VALUE"]))>0):

                                        $button_in_store = $button_in_store . '<a rel="nofollow" style="display: block;margin-top:10px;'.$center_button.'" onClick="ym(\'56316898\',\'reachGoal\', \'oformit\');" href="'  . $referer . '"  class="btn btn-default basket read_more"><span>' . GetMessage("OFO") . '</span></a>';
                                ...
                                if(isset($arResult['OFFERS'][0]['PROPERTIES']['URL']['VALUE'])):
                                    if(strlen($arResult['OFFERS'][0]['PROPERTIES']['URL']['VALUE'])>0):
                                        $strTemplate = str_replace('</li>','</li><div>' . $button_in_store . '</div>',$strTemplate);
                                    endif;
                                endif;

                                //if(strpos($APPLICATION->GetCurPage(), "finansy")!=false  || strpos($APPLICATION->GetCurPage(), "strakhovanie")!=false):
                                $strTemplate = str_replace("Партнер","",$strTemplate);
                                //endif;
                            ...
                            <div class="like_icons btn btn-default">
                                ...
                                <?if($arParams["DISPLAY_COMPARE"] == "Y"):?>
                                    <div class="compare_item_button o_<?=$arSKU["ID"];?>">
                                        <span title="<?=GetMessage('CATALOG_COMPARE')?>" class="compare_item to text <?=$arParams["TYPE_SKU"];?>" data-iblock="<?=$arParams["IBLOCK_ID"]?>" data-item="<?=$arSKU["ID"]?>" ><i></i><span><?=GetMessage('CT_BCE_CATALOG_COMPARE')?></span></span>
                                        <span title="<?=GetMessage('CATALOG_COMPARE_OUT')?>" class="compare_item in added text <?=$arParams["TYPE_SKU"];?>" style="display: none;" data-iblock="<?=$arParams["IBLOCK_ID"]?>" data-item="<?=$arSKU["ID"]?>"><i></i><span><?=GetMessage('CT_BCE_CATALOG_COMPARE')?></span></span>
                                    </div>
                                <?endif;?>
                            </div>
                            <?endif;?>
                        ...
                        <div class="tabs_section">
                            ...
                            <div class="tabs">
                                <ul class="nav nav-tabs">
                                    ...
                                    $usl = false;
                                    $treb = false;
                                    foreach ($arResult["OFFERS"][0]["PROPERTIES"] as $itemPropOf){
                                        $pos_treb = strpos($itemPropOf['NAME'], 'Требования.');
                                        if($pos_treb!== false){
                                            if(strlen($itemPropOf['VALUE'])>0 || count($itemPropOf['VALUE'])>1){
                                                $treb = true;
                                            }
                                        }
                                        $pos_usl  = strpos($itemPropOf['NAME'], 'Условия.');
                                        if($pos_usl!== false){
                                            if(strlen($itemPropOf['VALUE'])>0 || count($itemPropOf['VALUE'])>1){
                                                $usl = true;
                                            }
                                        }
                                        $pos_har  = strpos($itemPropOf['NAME'], 'Характеристики.');
                                        if($pos_har!== false){
                                            if(strlen($itemPropOf['VALUE'])>0 || count($itemPropOf['VALUE'])>1){
                                                $har = true;
                                            }
                                        }
                                        $pos_doc  = strpos($itemPropOf['NAME'], 'Документы.');
                                        if($pos_doc!== false){
                                            if(strlen($itemPropOf['VALUE'])>0 || count($itemPropOf['VALUE'])>1){
                                                $doc = true;
                                            }
                                        }
                                        $pos_ser  = strpos($itemPropOf['NAME'], 'Сервисы.');
                                        if($pos_ser!== false){
                                            if(strlen($itemPropOf['VALUE'])>0 || count($itemPropOf['VALUE'])>1){
                                                $ser = true;
                                            }
                                            ...
                                            <div class="detail_text">
                                                <table class="props_list">
                                                    ...
                                                    <?foreach ($arResult["OFFERS"][0]["PROPERTIES"] as $itemPropOf):?>
                                                        ...
                                                        $posPr = strpos($itemPropOf['NAME'], 'Сервисы.');
                                                        if($posPr===false){
                                                        }else {
                                                            if(strlen($itemPropOf['VALUE'])>0 || count($itemPropOf['VALUE'])>1):
                                                                ?>
                                                                <tr itemprop="additionalProperty">
                                                                    <td class="char_name">
                                                                        <div class="props_item ">
                                                                            <span itemprop="name"><?=str_replace('Сервисы.', '', $itemPropOf['NAME']) ?></span>
                                                                        </div>
                                                                    </td>
                                                                    <td class="char_value"><span itemprop="value"><?=getObProp($itemPropOf['NAME'], $itemPropOf['VALUE']) ?></span>
                                                                    </td>
                                                                </tr>
                                                                <?
                                                            endif;
                                        ...
                                        <div class="tab-pane <?=(!($iTab++) ? 'active' : '')?>" id="doc">
                                            ...
                                            <div class="detail_text">
                                                <table class="props_list">
                                                    ...
                                                    <?foreach ($arResult["OFFERS"][0]["PROPERTIES"] as $itemPropOf):?>
                                                        <?
                                                        $posPr = strpos($itemPropOf['NAME'], 'Документы.');
                                                        if($posPr===false){
                                                        }else {
                                                            if(strlen($itemPropOf['VALUE'])>0 || count($itemPropOf['VALUE'])>1):
                                                                if(strpos($itemPropOf['NAME'], ' от')!=false || strpos($itemPropOf['NAME'], ' до')!=false){continue;}
                                                                ?>
                                                                <tr itemprop="additionalProperty">
                                                                    <td class="char_name">
                                                                        <div class="props_item ">
                                                                            <span itemprop="name"><?=str_replace('Документы.', '', $itemPropOf['NAME']) ?></span>
                                                                        </div>
                                                                    </td>
                                                                    <td class="char_value"><span itemprop="value"><?=getObProp($itemPropOf['NAME'], $itemPropOf['VALUE']) ?></span>
                                                                    </td>
                                        ...
                                        <div class="tab-pane <?=(!($iTab++) ? 'active' : '')?>" id="har">
                                            ...
                                            <div class="detail_text">
                                                <table class="props_list">
                                                    ...
                                                    <?foreach ($arResult["OFFERS"][0]["PROPERTIES"] as $itemPropOf):?>
                                                        <?
                                                        $posPr = strpos($itemPropOf['NAME'], 'Характеристики.');
                                                        if($posPr===false){
                                                        }else {
                                                            //if(strlen($itemPropOf['VALUE'])>0 || count($itemPropOf['VALUE'])>1):
                                                            if(strpos($itemPropOf['NAME'], ' от')!=false || strpos($itemPropOf['NAME'], ' до')!=false){continue;}
                                                            if(in_array($itemPropOf['CODE'], $typeNoContact)){

                                                                if(!$start_property_nocontact){?>

                                                                    <tr itemprop="additionalProperty">
                                                                    <td class="char_name">
                                                                        <div class="props_item ">
                                                                            <span itemprop="name">Бесконтактная оплата</span>
                                                                        </div>
                                                                    </td>
                                                                    <?$start_property_nocontact = true;
                                                                }
                                                                if(strlen($value_property_nocontact)==0){
                                                                    $value_property_nocontact = str_replace('Характеристики.', '', $itemPropOf['NAME']);
                                                                }else{
                                                                    $value_property_nocontact = $value_property_nocontact . '; ' . str_replace('Характеристики.', '', $itemPropOf['NAME']);
                                                                }
                                                            }else {
                                                                if(strlen($itemPropOf['VALUE'])>0 || count($itemPropOf['VALUE'])>1):
                                                                    if($start_property_nocontact){?>
                                                                        <td class="char_value"><span itemprop="value"><?=$value_property_nocontact?></span></td>
                                                                        </tr>
                                                                        <?php
                                                                        $start_property_nocontact = false;
                                                                    }
                                                                    ...
                                                                    <tr itemprop="additionalProperty">
                                                                        <td class="char_name">
                                                                            <div class="props_item ">
                                                                                <span itemprop="name"><?=str_replace('Характеристики.', '', $itemPropOf['NAME']) ?></span>
                                                                            </div>
                                                                        </td>
                                                                        <td class="char_value"><span itemprop="value"><?=getObProp($itemPropOf['NAME'], $itemPropOf['VALUE']) ?></span>
                                                                        </td>
                                        ...
                                        <div class="tab-pane <?=(!($iTab++) ? 'active' : '')?>" id="usl">
                                            ...
                                            <div class="detail_text">
                                                <table class="props_list">
                                                    ...
                                                    <?foreach ($arResult["OFFERS"][0]["PROPERTIES"] as $itemPropOf):?>
                                                        <?
                                                        $posPr = strpos($itemPropOf['NAME'], 'Условия.');
                                                        if($posPr===false){
                                                        }else {
                                                            if(strlen($itemPropOf['VALUE'])>0 || count($itemPropOf['VALUE'])>1):
                                                                if(strpos($itemPropOf['NAME'], ' от')!=false || strpos($itemPropOf['NAME'], ' до')!=false){continue;}
                                                                ...
                                                                <tr itemprop="additionalProperty">
                                                                    <td class="char_name">
                                                                        <div class="props_item ">
                                                                            <span itemprop="name"><?=str_replace('Условия.', '', $itemPropOf['NAME']) ?></span>
                                                                        </div>
                                                                    </td>
                                                                    <td class="char_value"><span itemprop="value"><?=getObProp($itemPropOf['NAME'], $itemPropOf['VALUE']) ?></span>
                                                                    </td>
                                        ...
                                        <div class="tab-pane <?=(!($iTab++) ? 'active' : '')?>" id="treb">
                                            ...
                                            <div class="detail_text">
                                                <table class="props_list">
                                                    ...
                                                    <?foreach ($arResult["OFFERS"][0]["PROPERTIES"] as $itemPropOf):?>
                                                        <?
                                                        $posPr = strpos($itemPropOf['NAME'], 'Требования.');
                                                        if($posPr===false){
                                                        }else {
                                                            if(strlen($itemPropOf['VALUE'])>0 || count($itemPropOf['VALUE'])>1):
                                                                if(strpos($itemPropOf['NAME'], ' от')!=false || strpos($itemPropOf['NAME'], ' до')!=false){continue;}
                                                                ...
                                                                <tr itemprop="additionalProperty">
                                                                    <td class="char_name">
                                                                        <div class="props_item ">
                                                                            <span itemprop="name"><?=str_replace('Требования.','',$itemPropOf['NAME'])?></span>
                                                                        </div>
                                                                    </td>
                                                                    <td class="char_value"><span itemprop="value"><?=getObProp($itemPropOf['NAME'],$itemPropOf['VALUE'])?></span></td>
                                                                </tr>
                                                                <?
                                                            endif;
                                                        }
