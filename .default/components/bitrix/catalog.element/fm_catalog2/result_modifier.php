...
if(count($arrThisProp)>0)
{
    ...
    $arSelect = [ 'ID', 'NAME', 'CODE','PROPERTE_RAZDEL','PROPERTY_CODE_PROP', 'PROPERTY_INCLUDE_MAIN'];
    $res = CIBlockElement::GetList(array(),$arFilter,false,false,$arSelect);
    while($itemRes = $res->Fetch()) {

       //see($itemRes, true);
        $arrVisibleProperty[] = $itemRes['PROPERTY_CODE_PROP_VALUE'];
        if(!empty($itemRes['PROPERTY_INCLUDE_MAIN_VALUE'])){
            $arrIncludeMainID[] = $itemRes['ID'];
        }

    }
    $arrClearIncludeMainID = array_unique($arrIncludeMainID);

    $arResult['INCLUDE_MAIN'] = $arrClearIncludeMainID;
    //see($arResult['INCLUDE_MAIN'], true);
